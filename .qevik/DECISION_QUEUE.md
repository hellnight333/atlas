...
| DQ-010 | Does one address the guard refuses fail the whole verification pass? | Keep it — any address-guard refusal fails the run / fail only the refused target and let the rest of the pass stand / fail the run but record it as a different outcome from "nothing was fetched" | None, and deliberately none. `toolrunner._fetch` fails the step on any address-guard refusal because "the recipe was pointed somewhere it must not go, and that is worth stopping for". The consequence, measured: `mission-9403ed56cc88` read 40 sites, wrote 7 observation records, 3 comparisons and signals for 10 businesses, and is recorded as `failed`. Whether that run succeeded is a judgement about what a run *means*, not a defect to fix — the record now states the cause and that the output is live, and changes nothing about the guard | — | `mission/toolrunner.py:_fetch`; `mission-9403ed56cc88`; E-31 | 2026-09-01 | OPEN |
