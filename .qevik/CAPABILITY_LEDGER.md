...
| C-48 | Observation backlog | Control plane | ✓ | ✓ | ✓ | ✓ | — | — | — | TEST-VERIFIED | `audit_backlog()` beside freshness on the coverage screen: 40 sites a night over 359 is a 9-night sweep, so most observations are older than a week by schedule. Reports the cadence, changes nothing. Separates a stall and a rotation that skips from a queue, and counts sites the sweep already reached and could not re-read. Health is read only from rows the pass signs, so a hand-run reconcile cannot certify a stalled pass; the population is the addresses it can actually fetch. **Not deployed; no production run has been read through it** |
