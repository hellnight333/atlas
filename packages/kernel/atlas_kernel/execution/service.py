...
def execute(recommendation: Recommendation, *, approved: bool, research: dict,
    ...
    return ExecutionOutcome(
        ...
        capability_id=recommendation.capability_id,
        estimated_units=offer.estimated_units if offer else 0,
        # Only when the capability reported it. `None` means nobody counted,
        # which must not later be read as zero work.
        actual_units=provenance.get("units") if isinstance(provenance, dict) else None,
