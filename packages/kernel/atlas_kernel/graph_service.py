...
class GraphService:
    ...
    def neighbors(self, node_id: str) -> list[KnowledgeNode]:
        ...
        neighbor_ids = {edge.to_node for edge in edges if edge.from_node == node_id} | {
            edge.from_node for edge in edges if edge.to_node == node_id
        }
        return [
            node
            for node in (self.repository.get_graph_node(item) for item in neighbor_ids)
            if node is not None
        ]
    ...
    def asset_lineage(self, asset_id: str) -> KnowledgeGraph:
        nodes = [
            node
            for node in self.repository.list_graph_nodes()
            if node.source_id == asset_id or node.id == asset_id
        ]
        node_ids = {node.id for node in nodes}
        edges = [
            edge
    ...
    def context_bundle(self, project_id: str) -> ContextBundle:
        ...
        bundle = ContextBundle(
            ...
            recent_chats=[
                item.model_dump() for item in self.repository.list_chat_conversations(project_id)
            ],
            related_assets=[
                item.model_dump() for item in self.repository.list_assets(project_id=project_id)
            ],
            research_findings=[
                item.model_dump() for item in self.repository.list_research_sessions(project_id)
            ],
            reviews=[
                item.model_dump() for item in self.repository.list_review_sessions(project_id)
            ],
            agent_history=[item.model_dump() for item in self.repository.list_agents(project_id)],
            workflow_history=[
                item.model_dump() for item in self.repository.list_workflows(project_id)
            ],
            execution_history=[
                item.model_dump(mode="json") for item in self.repository.list_runtime_executions()
            ],
            referenced_images=[
                item.model_dump()
                for item in self.repository.list_assets(project_id=project_id)
                if item.type == "image"
            ],
            referenced_reports=[
                item.model_dump()
    ...
    def create_snapshot(self, scope_type: str, scope_id: str) -> GraphSnapshot:
        graph = (
            self.project_subgraph(scope_id)
            if scope_type == "project"
            else KnowledgeGraph(
                nodes=self.repository.list_graph_nodes(), edges=self.repository.list_graph_edges()
            )
        )
        ...
        self.event_bus.publish(
            GraphSnapshotCreated(snapshot_id=snapshot.id, scope_type=scope_type, scope_id=scope_id)
        )
        return snapshot
    ...
    def materialize_project_graph(self, project_id: str) -> KnowledgeGraph:
        ...
        for asset in assets:
            self.create_node(
                KnowledgeNode(
                    id=asset.id,
                    node_type=asset.type.title(),
                    label=asset.metadata.get("title") or asset.id,
                    project_id=project_id,
                    source_id=asset.id,
                    metadata={"uri": asset.uri},
                )
            )
            self.create_edge(
                KnowledgeEdge(
                    relationship=RelationshipType.BELONGS_TO, from_node=asset.id, to_node=project_id
                )
            )
        ...
        for chat in chats:
            ...
            self.create_edge(
                KnowledgeEdge(
                    relationship=RelationshipType.BELONGS_TO, from_node=chat.id, to_node=project_id
                )
            )
            ...
            if chat.response_asset_id:
                self.create_edge(
                    KnowledgeEdge(
                        relationship=RelationshipType.REFERENCES,
                        from_node=chat.id,
                        to_node=chat.response_asset_id,
                    )
                )
        ...
        for review in reviews:
            ...
            if review.asset_id:
                self.create_edge(
                    KnowledgeEdge(
                        relationship=RelationshipType.REVIEWED_BY,
                        from_node=review.asset_id,
                        to_node=review.id,
                    )
                )
        ...
        for run in runs:
            ...
            self.create_edge(
                KnowledgeEdge(
                    relationship=RelationshipType.EXECUTED_BY, from_node=run.id, to_node=project_id
                )
            )
            if run.workflow_id:
                self.create_edge(
                    KnowledgeEdge(
                        relationship=RelationshipType.GENERATED_FOR,
                        from_node=run.id,
                        to_node=run.workflow_id,
                    )
                )
