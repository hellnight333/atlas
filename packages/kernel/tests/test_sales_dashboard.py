...
def test_a_sample_is_never_presented_as_client_work() -> None:
    assert set(sales.SAMPLE_FOR.values()) <= set(sales.SAMPLE_WHY)
    html = CONTROL_HTML.read_text(encoding="utf-8")
    assert "Qevik has no customers" in html
    assert "never a client" in html


def test_every_industry_maps_to_a_sample_that_exists() -> None:
    samples = Path(__file__).resolve().parents[3] / "apps" / "samples"
    for category, slug in sales.SAMPLE_FOR.items():
        if slug == "sample":
            continue                      # generated by the dental vertical
        assert (samples / slug.replace("sample-", "") / "index.html").exists(), \
            f"{category} points at {slug}, which is not built"
