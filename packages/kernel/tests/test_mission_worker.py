...
def test_the_worker_reads_the_vault_the_credential_centre_writes(tmp_path,
                                                                  monkeypatch
                                                                  ) -> None:
    """These were two different files.

    `from_environment` writes `<QEVIK_STATE>/vault.json`; the worker read
    `<vault_root>/credentials.json`. A key entered in the Centre was invisible
    to the worker, and the worker's refusal would have read as "no credential
    configured" to an operator looking at a Centre showing one connected.
    """
    ...
    state.mkdir()
    written = state / "vault.json"
    # Both halves, as the Centre writes them: the vault holds the secret and the
    # timeline holds the record. A worker with only one of them finds nothing,
    # which is the second half of the same bug.
    records = Timeline(state / "credentials.jsonl")
    CredentialService(Vault(FileSecretStore(written)), events=records.read(),
                      sink=records.append).store(
        provider="qwen", tenant="t1", secret="sk-not-a-real-key")
    ...
    # Pointed at the file, and at the directory containing it. Both must find
    # what the Centre wrote — an operator should not have to know which shape
    # the flag wants.
    for given in (written, state):
        resolved = worker._vault_file(given)
        seen = CredentialService(Vault(FileSecretStore(resolved)),
                                 events=records.read(), sink=records.append)
        assert seen.resolve(provider="qwen",
                            tenant="t1") == "sk-not-a-real-key", given

    # The negative control: the old path finds nothing, which is precisely what
    # the operator experienced.
    stale = CredentialService(Vault(FileSecretStore(state / "credentials.json")),
                              events=records.read(), sink=records.append)
    with pytest.raises(Exception):  # noqa: B017 - CredentialMissing
        stale.resolve(provider="qwen", tenant="t1")
